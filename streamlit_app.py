...
st.set_page_config(page_title="PMO Job Dashboard", page_icon="💼", layout="centered")

st.title("💼 PMO & Project Management Dashboard")
st.caption("Live search for postings in the last 24 hours (US)")

def is_priority(job):
    """Returns True if job matches Chicago or Remote criteria."""
...
def fetch_linkedin_jobs(max_pages=2):
    ...
    for page in range(max_pages):
        ...
        try:
            ...
            if response.status_code != 200:
                st.error(f"LinkedIn HTTP Status: {response.status_code} (Search throttled)")
                break
            ...
            if not cards:
                if page == 0:
                    st.warning("No listings returned right now. Try refreshing shortly.")
                break
    ...
    # Prioritize Chicago / Remote jobs
    all_jobs.sort(key=lambda j: not is_priority(j))
    return all_jobs

# Control refresh logic using Streamlit state
if "jobs" not in st.session_state or st.button("🔄 Refresh Listings", type="primary"):
    with st.spinner("Fetching latest postings from LinkedIn..."):
        st.session_state.jobs = fetch_linkedin_jobs(max_pages=2)
